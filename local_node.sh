...
LOGLEVEL="info"
# Set dedicated home directory for the sequencerd instance
HOMEDIR="$HOME/.tmp-sequencerd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	sequencerd config keyring-backend $KEYRING --home "$HOMEDIR"
	sequencerd config chain-id $CHAINID --home "$HOMEDIR"

	# If keys exist they should be deleted
	for KEY in "${KEYS[@]}"; do
		sequencerd keys add "$KEY" --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
	done

	# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
	sequencerd init $MONIKER -o --chain-id $CHAINID --home "$HOMEDIR"
	...
	# Set block time
#  jq '.consensus_params["block"]["time_iota_ms"]="16"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	claims_key=${KEYS[0]}
	node_address=$(sequencerd keys show "$claims_key" --keyring-backend $KEYRING --home "$HOMEDIR" | grep "address" | cut -c12-)
...
#  sed -i '' 's/create_empty_blocks_interval = "0s"/create_empty_blocks_interval = "30s"/g' "$CONFIG"
	...
	for KEY in "${KEYS[@]}"; do
		sequencerd add-genesis-account "$KEY" 100000000000000000000000000aevmos --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	sequencerd gentx "${KEYS[0]}" 1000000000000000000000aevmos --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
	## In case you want to create multiple validators at genesis
	## 1. Back to `sequencerd keys add` step, init more keys
	## 2. Back to `sequencerd add-genesis-account` step, add balance for those
	## 3. Clone this ~/.sequencerd home directory into some others, let's say `~/.clonedEvmosd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.sequencerd/config/gentx`

	# Collect genesis tx
	sequencerd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	sequencerd validate-genesis --home "$HOMEDIR"
...
sequencerd start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aevmos --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"
